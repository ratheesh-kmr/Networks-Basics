...
#define MULTICAST_PORT 12347        // Multicast port
#define MAX_TEAMS 1
#define TIME_TO_ANSWER 100           // Time in seconds
...
int main() {
    ...
    // Set reuse address option (to allow multiple bindings to same address/port)
    int reuse = 1;
    ...
    if (bind(udp_sock, (struct sockaddr*)&udp_addr, sizeof(udp_addr)) < 0) {
        std::cerr << "Failed to bind UDP socket. Port might be in use.\n";
        close(sockfd);
        close(udp_sock);
        return -1;
    }
